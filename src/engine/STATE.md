...
Écrire `walk.py` dans sa forme minimale : le prédicat `is_verifiable(node)`, la scission bornée en profondeur et en largeur, le blocage avec cause typée — testé **uniquement** contre les doubles de `verifier`, `workspace` et `journal`, sans Prefect. C'est la moitié verticale du jalon « premier vert ».
