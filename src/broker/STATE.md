...
Écrire `git.py` : `repo_fact` — `git status --porcelain=v1 -z -uall` plus `git diff --binary`, avec les deux côtés d'un rename, rendant le `RepoFact` typé que `verifier` consomme. Une ligne malformée doit être une erreur, pas un silence.
